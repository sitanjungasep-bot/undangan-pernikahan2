...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        if (name) {
            // Display guest name in header section
            const guestNameHeader = document.getElementById('guest-name-header');
            if (guestNameHeader) {
                const headerDiv = document.createElement('div');
                const headerTemplate = `<small class="d-block text-secondary mb-2" style="font-size: 0.95rem;">Kepada Yth Bapak/Ibu/Saudara/i</small><p class="m-0 p-0 font-esthetic" style="font-size: 2rem; font-weight: 600;">${util.escapeHtml(name)}</p>`;
                util.safeInnerHTML(headerDiv, headerTemplate);
                guestNameHeader.appendChild(headerDiv);
            }
